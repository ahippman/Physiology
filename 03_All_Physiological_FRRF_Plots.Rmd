...
p <- ggplot(mydata.mean.stderr, aes(x=Treatment, y=mean.X14C.per.Chla.at.155uE, fill = Species)) + 
  labs(title = bquote("14C uptake @ 155"~mu~"E" ), y=bquote("mg C mg Chl" ~italic(a)^-1~h-1 ), x= "")
...
``` {r  plot mol.O2.mol.Chla.h.mod, eval=TRUE, echo=FALSE}
p <- ggplot(mydata.mean.stderr, aes(x=Treatment, y=mean.mol.O2.mol.Chla.h.mod, fill = Species)) + 
...
p27 <- p + geom_bar(color = "black", position=position_dodge(), stat="identity", width = .6) +
  scale_fill_manual(values = Species_colours) +
  geom_errorbar(aes(ymin = mean.mol.O2.mol.Chla.h.mod - sd.err.mol.O2.mol.Chla.h.mod, ymax = mean.mol.O2.mol.Chla.h.mod + sd.err.mol.O2.mol.Chla.h.mod),
                  width = .15,                    # Width of the error bars
                  position = position_dodge(.6), #so the error bars are dodged to the side
                  colour = "black") +
  cleanup +
...
```{r plot_file_mol.O2.mol.Chla.h.mod, eval=FALSE, echo=FALSE}
png('plots/ALL_Phys_Barplots/lowCu_mol.O2.mol.Chla.h.mod.png', width=680, height = 480, units ="px", pointsize = 12) #directly puts plot into file without showing it in plot window first!
