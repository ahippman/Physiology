...
``` {r  plot cell.SA.Vol.ratio, eval=T, echo=T}
...
``` {r  plot mol.O2.mol.Chla.h.mod, eval=FALSE, echo=FALSE}
...
``` {r  plot X14C.molC.per.mol.Chla.h, eval=FALSE, echo=FALSE}
...
# p29 - PQ.Chla.all
``` {r  plot PQ.Chla.all, eval=FALSE, echo=FALSE}
p <- ggplot(mydata.mean.stderr, aes(x=Treatment, y=mean.PQ.Chla.all, fill = Species)) + 
  labs(title = bquote("Photosynthetic quotient - PQ - all - dont use" ), y=bquote("mol " ~ O[2] ~ "mol " ~ C^-1), x= "")
...
``` {r  plot PQ.Chla.all.delete, eval=FALSE, echo=FALSE}
p <- ggplot(mydata.mean.stderr, aes(x=Treatment, y=mean.PQ.Chla.all.delete, fill = Species)) + 
  labs(title = bquote("Photosynthetic quotient" ~ PQ ), y=bquote("mol " ~ O[2] ~ "mol" ~ C^-1), x= "")
