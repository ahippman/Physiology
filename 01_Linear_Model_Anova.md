...
* Overview of [My Method](#MyMethod) to Analyze my Physiological Data
  ...
  + [14C uptake - pmax (per Chl a)](#14C.pmax.chla)
...
```r
p <- ggplot (mydata , aes(Treatment, X14C.per.Chla.pmax))
p + geom_point(aes(group=Merged, colour=Species), size = 3, alpha=0.5)+
  labs(title="14C uptake per Chla - pmax", x ="")+
  guides(alpha = "none", size = "none", shape = "none")+
  expand_limits(y=0)
```
...
```r
lm_all_14C.pmax.chla <- lm(data = mydata, X14C.per.Chla.pmax ~ (Species * Fe.level * Cu.level))
summary(lm_all_14C.pmax.chla) # we need to look at the actual anova to know where we need to look further
```

```
## 
## Call:
## lm(formula = X14C.per.Chla.pmax ~ (Species * Fe.level * Cu.level), 
##     data = mydata)
## 
## Residuals:
##     Min      1Q  Median      3Q     Max 
## -1.0400 -0.4249 -0.1253  0.2826  1.6104 
## 
## Coefficients:
##                                        Estimate Std. Error t value
## (Intercept)                              3.8894     0.4826   8.060
## SpeciesTO 1005                          -0.4311     0.6824  -0.632
## Fe.levellow                             -1.3031     0.6824  -1.909
## Cu.levellow                             -2.9243     0.6824  -4.285
## SpeciesTO 1005:Fe.levellow               1.8481     0.9651   1.915
## SpeciesTO 1005:Cu.levellow               3.8617     0.9651   4.001
## Fe.levellow:Cu.levellow                  2.1576     0.9651   2.236
## SpeciesTO 1005:Fe.levellow:Cu.levellow  -4.7206     1.3649  -3.459
##                                        Pr(>|t|)    
## (Intercept)                            5.03e-07 ***
## SpeciesTO 1005                         0.536457    
## Fe.levellow                            0.074299 .  
## Cu.levellow                            0.000568 ***
## SpeciesTO 1005:Fe.levellow             0.073548 .  
## SpeciesTO 1005:Cu.levellow             0.001029 ** 
## Fe.levellow:Cu.levellow                0.039981 *  
## SpeciesTO 1005:Fe.levellow:Cu.levellow 0.003234 ** 
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
## 
## Residual standard error: 0.8358 on 16 degrees of freedom
## Multiple R-squared:  0.7289,	Adjusted R-squared:  0.6103 
## F-statistic: 6.145 on 7 and 16 DF,  p-value: 0.001304
```

```r
anova(lm_all_14C.pmax.chla) 
```

```
## Analysis of Variance Table
## 
## Response: X14C.per.Chla.pmax
##                           Df  Sum Sq Mean Sq F value   Pr(>F)   
## Species                    1  9.2795  9.2795 13.2836 0.002183 **
## Fe.level                   1  1.3845  1.3845  1.9819 0.178321   
## Cu.level                   1  7.1925  7.1925 10.2961 0.005476 **
## Species:Fe.level           1  0.3934  0.3934  0.5632 0.463866   
## Species:Cu.level           1  3.3812  3.3812  4.8402 0.042843 * 
## Fe.level:Cu.level          1  0.0616  0.0616  0.0882 0.770269   
## Species:Fe.level:Cu.level  1  8.3564  8.3564 11.9622 0.003234 **
## Residuals                 16 11.1770  0.6986                    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```

there is a three way Species:Fe.level:Cu.level* interaction. Let's leave everything in, again:



```r
lm_14C.pmax.chla <- lm (data = mydata, X14C.per.Chla.pmax ~ (Species * Fe.level * Cu.level))
summary(lm_14C.pmax.chla) # we need to look at the actual anova to knwo where we need to look further
```
...
Looking at the ANOVA table for the linear model that tests for all interactions for the data __14C.pmax.chla__, we see the following significant factors:


* __Species__ alone has a significant effect 
* __Fe. level__ alone has no significant effect  
* __Cu level__ alone has a significant effect 
* there is  an __interaction__ between __Species and Cu level__ 
* there is  __ a three-way interaction__ between __Species, Fe level and Cu level__ 



### Using the `phia` package to look into significant effects regarding 14C.pmax.chla

Again, any main effects ( none here) that are included in interacting effects will be looked at through pairwise comparisons of the interacting factors


```r
(C14.alpha.cell.means <- interactionMeans(lm_14C.pmax.chla))
```

```
##   Species Fe.level Cu.level adjusted mean std. error
## 1 TO 1003     high     high     3.8893941  0.4825502
## 2 TO 1005     high     high     3.4582525  0.4825502
## 3 TO 1003      low     high     2.5863109  0.4825502
## 4 TO 1005      low     high     4.0033082  0.4825502
## 5 TO 1003     high      low     0.9650445  0.4825502
## 6 TO 1005     high      low     4.3955571  0.4825502
## 7 TO 1003      low      low     1.8195448  0.4825502
## 8 TO 1005      low      low     2.3776328  0.4825502
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed="Species", across=c("Fe.level", "Cu.level")) # no significance
```
...
## P-value adjustment method: holm
##             Value Df Sum of Sq      F  Pr(>F)  
## TO 1003    2.1576  1    3.4914 4.9979 0.03998 *
## TO 1005   -2.5630  1    4.9266 7.0525 0.03454 *
...
```r
testInteractions(lm_14C.pmax.chla, fixed="Cu.level", across=c("Species", "Fe.level")) # no significance
```
...
## P-value adjustment method: holm
##             Value Df Sum of Sq      F  Pr(>F)  
## high       1.8481  1    2.5617 3.6671 0.07355 .
##  low      -2.8724  1    6.1881 8.8583 0.01782 *
## Residuals         16   11.1770                 
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed="Fe.level", across=c("Species", "Cu.level"))
```
...
## P-value adjustment method: holm
##             Value Df Sum of Sq      F   Pr(>F)   
## high       3.8617  1   11.1843 16.010 0.002058 **
##  low      -0.8589  1    0.5533  0.792 0.386669   
## Residuals         16   11.1770                   
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed="Species", across="Cu.level") # no significance
```
...
## P-value adjustment method: holm
##             Value Df Sum of Sq       F   Pr(>F)   
## TO 1003   1.84556  1   10.2183 14.6275 0.002986 **
## TO 1005   0.34419  1    0.3554  0.5087 0.485957   
## Residuals         16   11.1770                    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed = "Cu.level", across="Species")
```
...
## P-value adjustment method: holm
##              Value Df Sum of Sq       F   Pr(>F)   
## high      -0.49293  1    0.7289  1.0435 0.322213   
##  low      -1.99430  1   11.9317 17.0803 0.001562 **
## Residuals          16   11.1770                    
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
let's check the 2 fixed and against one other, especially in regards to low Cu

```r
testInteractions(lm_14C.pmax.chla, fixed= c( "Species", "Fe.level"), across= "Cu.level") # lowCu in TO1003 **
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed= c( "Fe.level", "Cu.level"), across="Species") # low Cu *** diff between strains
```
...
## P-value adjustment method: holm
##               Value Df Sum of Sq       F    Pr(>F)    
## high : high  0.4311  1    0.2788  0.3991 0.8509701    
##  low : high -1.4170  1    3.0118  4.3114 0.1629910    
## high :  low -3.4305  1   17.6526 25.2699 0.0004958 ***
##  low :  low -0.5581  1    0.4672  0.6688 0.8509701    
## Residuals           16   11.1770                      
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
```r
testInteractions(lm_14C.pmax.chla, fixed=c( "Species", "Cu.level"), across= "Fe.level") 
```
...
```r
testInteractions(lm_14C.155uEinstein.chla, fixed= c( "Species", "Fe.level"), across= "Cu.level") # lowCu in TO1003
```
...
```r
testInteractions(lm_14C.155uEinstein.chla, fixed= c( "Fe.level", "Cu.level"), across="Species") # low Cu diff between strains
```
...
## P-value adjustment method: holm
##                Value Df Sum of Sq       F   Pr(>F)    
## high : high  0.15667  1    0.0368  0.1402 0.712959    
##  low : high -0.43667  1    0.2860  1.0895 0.624201    
## high :  low -2.01000  1    6.0601 23.0841 0.000778 ***
##  low :  low -0.58667  1    0.5163  1.9665 0.539758    
## Residuals            16    4.2004                     
## ---
## Signif. codes:  0 '***' 0.001 '**' 0.01 '*' 0.05 '.' 0.1 ' ' 1
```
...
```r
testInteractions(lm_14C.155uEinstein.chla, fixed=c( "Species", "Cu.level"), across= "Fe.level") #no significane
```
...
###Summary 14C per Chla at 155 uEinstein - 


TO 1003 decreases 14C uptake per Chla at 155 uEinstein significantly in response to low Cu. And under low Cu the results are significantly different for both strains. low Fe alone does not have an effect on 14C uptake when normalized to Chla. lowFeCu has only on effect on TO1003. To 1005 is not affected
...
lm_Chla.cell <- lm (data = mydata, Chla.per.cell.pg.cell ~ (Species * Fe.level * Cu.level)) # Yes, this is the same as before. I leave it as is
#~~~~~~~~~~~~~~
anova_Chla.cell <- anova(lm_Chla.cell)
anova_Chla.cell <- anova_Chla.cell %>% 
  mutate(Analysis = "anova", Data = "Chla.cell", Effects_Tested = rownames(anova_Chla.cell))

anova_Chla.cell <- data.frame(anova_Chla.cell[, 6:8], anova_Chla.cell[,1:5])
#~~~~~~~~~~~~~~
phia_Chla.cell_Species_Cu_vs_Fe <- testInteractions(lm_Chla.cell, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_Chla.cell_Species_Cu_vs_Fe <- phia_Chla.cell_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_Chla.cell_Species_Cu_vs_Fe))

phia_Chla.cell_Species_Cu_vs_Fe <- data.frame(phia_Chla.cell_Species_Cu_vs_Fe[,6:9], phia_Chla.cell_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_Chla.cell_Species_Fe_vs_Cu <- testInteractions(lm_Chla.cell, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_Chla.cell_Species_Fe_vs_Cu <- phia_Chla.cell_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_Chla.cell_Species_Fe_vs_Cu))

phia_Chla.cell_Species_Fe_vs_Cu <- data.frame(phia_Chla.cell_Species_Fe_vs_Cu[,6:9], phia_Chla.cell_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_Chla.cell_Fe_Cu_vs_Species <- testInteractions(lm_Chla.cell, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_Chla.cell_Fe_Cu_vs_Species <- phia_Chla.cell_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_Chla.cell_Fe_Cu_vs_Species))

phia_Chla.cell_Fe_Cu_vs_Species <- data.frame(phia_Chla.cell_Fe_Cu_vs_Species[,6:9], phia_Chla.cell_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_Chla.cell_Fe_vs_Cu <- testInteractions(lm_Chla.cell, fixed="Fe.level", across="Cu.level")
phia_Chla.cell_Fe_vs_Cu <- phia_Chla.cell_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_Chla.cell_Fe_vs_Cu))

phia_Chla.cell_Fe_vs_Cu <- data.frame(phia_Chla.cell_Fe_vs_Cu[,6:9], phia_Chla.cell_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_Chla.cell_Cu_vs_Fe <- testInteractions(lm_Chla.cell, fixed="Cu.level", across="Fe.level")
phia_Chla.cell_Cu_vs_Fe <- phia_Chla.cell_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_Chla.cell_Cu_vs_Fe))

phia_Chla.cell_Cu_vs_Fe <- data.frame(phia_Chla.cell_Cu_vs_Fe[,6:9], phia_Chla.cell_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_Chla.cell_Species_vs_Fe <- testInteractions(lm_Chla.cell, fixed="Species", across="Fe.level")
phia_Chla.cell_Species_vs_Fe <- phia_Chla.cell_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Species_vs_Fe", tested = rownames(phia_Chla.cell_Species_vs_Fe))

phia_Chla.cell_Species_vs_Fe <- data.frame(phia_Chla.cell_Species_vs_Fe[,6:9], phia_Chla.cell_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_Chla.cell_Fe_vs_Species <- testInteractions(lm_Chla.cell, fixed="Fe.level", across="Species")
phia_Chla.cell_Fe_vs_Species <- phia_Chla.cell_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Fe_vs_Species", tested = rownames(phia_Chla.cell_Fe_vs_Species))

phia_Chla.cell_Fe_vs_Species <- data.frame(phia_Chla.cell_Fe_vs_Species[,6:9], phia_Chla.cell_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_Chla.cell_Species_vs_Cu <- testInteractions(lm_Chla.cell, fixed="Species", across="Cu.level")
phia_Chla.cell_Species_vs_Cu <- phia_Chla.cell_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Species_vs_Cu", tested = rownames(phia_Chla.cell_Species_vs_Cu))

phia_Chla.cell_Species_vs_Cu <- data.frame(phia_Chla.cell_Species_vs_Cu[,6:9], phia_Chla.cell_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_Chla.cell_Cu_vs_Species <- testInteractions(lm_Chla.cell, fixed="Cu.level", across="Species")
phia_Chla.cell_Cu_vs_Species <- phia_Chla.cell_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "Chla.cell",test_intrctn = "Cu_vs_Species", tested = rownames(phia_Chla.cell_Cu_vs_Species))

phia_Chla.cell_Cu_vs_Species <- data.frame(phia_Chla.cell_Cu_vs_Species[,6:9], phia_Chla.cell_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_Chla.cell table

all_phia_Chla.cell <- bind_rows(phia_Chla.cell_Cu_vs_Species, phia_Chla.cell_Species_vs_Cu, phia_Chla.cell_Fe_vs_Species, phia_Chla.cell_Species_vs_Fe, phia_Chla.cell_Cu_vs_Fe, phia_Chla.cell_Fe_vs_Cu, phia_Chla.cell_Fe_Cu_vs_Species, phia_Chla.cell_Species_Fe_vs_Cu, phia_Chla.cell_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_cell.size <- anova(lm_cell.size)
anova_cell.size <- anova_cell.size %>% 
  mutate(Analysis = "anova", Data = "cell.size", Effects_Tested = rownames(anova_cell.size))

anova_cell.size <- data.frame(anova_cell.size[, 6:8], anova_cell.size[,1:5])
#~~~~~~~~~~~~~~
phia_cell.size_Species_Cu_vs_Fe <- testInteractions(lm_cell.size, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_cell.size_Species_Cu_vs_Fe <- phia_cell.size_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_cell.size_Species_Cu_vs_Fe))

phia_cell.size_Species_Cu_vs_Fe <- data.frame(phia_cell.size_Species_Cu_vs_Fe[,6:9], phia_cell.size_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_cell.size_Species_Fe_vs_Cu <- testInteractions(lm_cell.size, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_cell.size_Species_Fe_vs_Cu <- phia_cell.size_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_cell.size_Species_Fe_vs_Cu))

phia_cell.size_Species_Fe_vs_Cu <- data.frame(phia_cell.size_Species_Fe_vs_Cu[,6:9], phia_cell.size_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_cell.size_Fe_Cu_vs_Species <- testInteractions(lm_cell.size, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_cell.size_Fe_Cu_vs_Species <- phia_cell.size_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_cell.size_Fe_Cu_vs_Species))

phia_cell.size_Fe_Cu_vs_Species <- data.frame(phia_cell.size_Fe_Cu_vs_Species[,6:9], phia_cell.size_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_cell.size_Fe_vs_Cu <- testInteractions(lm_cell.size, fixed="Fe.level", across="Cu.level")
phia_cell.size_Fe_vs_Cu <- phia_cell.size_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_cell.size_Fe_vs_Cu))

phia_cell.size_Fe_vs_Cu <- data.frame(phia_cell.size_Fe_vs_Cu[,6:9], phia_cell.size_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_cell.size_Cu_vs_Fe <- testInteractions(lm_cell.size, fixed="Cu.level", across="Fe.level")
phia_cell.size_Cu_vs_Fe <- phia_cell.size_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_cell.size_Cu_vs_Fe))

phia_cell.size_Cu_vs_Fe <- data.frame(phia_cell.size_Cu_vs_Fe[,6:9], phia_cell.size_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_cell.size_Species_vs_Fe <- testInteractions(lm_cell.size, fixed="Species", across="Fe.level")
phia_cell.size_Species_vs_Fe <- phia_cell.size_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Species_vs_Fe", tested = rownames(phia_cell.size_Species_vs_Fe))

phia_cell.size_Species_vs_Fe <- data.frame(phia_cell.size_Species_vs_Fe[,6:9], phia_cell.size_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_cell.size_Fe_vs_Species <- testInteractions(lm_cell.size, fixed="Fe.level", across="Species")
phia_cell.size_Fe_vs_Species <- phia_cell.size_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Fe_vs_Species", tested = rownames(phia_cell.size_Fe_vs_Species))

phia_cell.size_Fe_vs_Species <- data.frame(phia_cell.size_Fe_vs_Species[,6:9], phia_cell.size_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_cell.size_Species_vs_Cu <- testInteractions(lm_cell.size, fixed="Species", across="Cu.level")
phia_cell.size_Species_vs_Cu <- phia_cell.size_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Species_vs_Cu", tested = rownames(phia_cell.size_Species_vs_Cu))

phia_cell.size_Species_vs_Cu <- data.frame(phia_cell.size_Species_vs_Cu[,6:9], phia_cell.size_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_cell.size_Cu_vs_Species <- testInteractions(lm_cell.size, fixed="Cu.level", across="Species")
phia_cell.size_Cu_vs_Species <- phia_cell.size_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.size",test_intrctn = "Cu_vs_Species", tested = rownames(phia_cell.size_Cu_vs_Species))

phia_cell.size_Cu_vs_Species <- data.frame(phia_cell.size_Cu_vs_Species[,6:9], phia_cell.size_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_cell.size table

all_phia_cell.size <- bind_rows(phia_cell.size_Cu_vs_Species, phia_cell.size_Species_vs_Cu, phia_cell.size_Fe_vs_Species, phia_cell.size_Species_vs_Fe, phia_cell.size_Cu_vs_Fe, phia_cell.size_Fe_vs_Cu, phia_cell.size_Fe_Cu_vs_Species, phia_cell.size_Species_Fe_vs_Cu, phia_cell.size_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_cell.vol <- anova(lm_cell.vol)
anova_cell.vol <- anova_cell.vol %>% 
  mutate(Analysis = "anova", Data = "cell.vol", Effects_Tested = rownames(anova_cell.vol))

anova_cell.vol <- data.frame(anova_cell.vol[, 6:8], anova_cell.vol[,1:5])
#~~~~~~~~~~~~~~
phia_cell.vol_Species_Cu_vs_Fe <- testInteractions(lm_cell.vol, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_cell.vol_Species_Cu_vs_Fe <- phia_cell.vol_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_cell.vol_Species_Cu_vs_Fe))

phia_cell.vol_Species_Cu_vs_Fe <- data.frame(phia_cell.vol_Species_Cu_vs_Fe[,6:9], phia_cell.vol_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_cell.vol_Species_Fe_vs_Cu <- testInteractions(lm_cell.vol, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_cell.vol_Species_Fe_vs_Cu <- phia_cell.vol_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_cell.vol_Species_Fe_vs_Cu))

phia_cell.vol_Species_Fe_vs_Cu <- data.frame(phia_cell.vol_Species_Fe_vs_Cu[,6:9], phia_cell.vol_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_cell.vol_Fe_Cu_vs_Species <- testInteractions(lm_cell.vol, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_cell.vol_Fe_Cu_vs_Species <- phia_cell.vol_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_cell.vol_Fe_Cu_vs_Species))

phia_cell.vol_Fe_Cu_vs_Species <- data.frame(phia_cell.vol_Fe_Cu_vs_Species[,6:9], phia_cell.vol_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_cell.vol_Fe_vs_Cu <- testInteractions(lm_cell.vol, fixed="Fe.level", across="Cu.level")
phia_cell.vol_Fe_vs_Cu <- phia_cell.vol_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_cell.vol_Fe_vs_Cu))

phia_cell.vol_Fe_vs_Cu <- data.frame(phia_cell.vol_Fe_vs_Cu[,6:9], phia_cell.vol_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_cell.vol_Cu_vs_Fe <- testInteractions(lm_cell.vol, fixed="Cu.level", across="Fe.level")
phia_cell.vol_Cu_vs_Fe <- phia_cell.vol_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_cell.vol_Cu_vs_Fe))

phia_cell.vol_Cu_vs_Fe <- data.frame(phia_cell.vol_Cu_vs_Fe[,6:9], phia_cell.vol_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_cell.vol_Species_vs_Fe <- testInteractions(lm_cell.vol, fixed="Species", across="Fe.level")
phia_cell.vol_Species_vs_Fe <- phia_cell.vol_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Species_vs_Fe", tested = rownames(phia_cell.vol_Species_vs_Fe))

phia_cell.vol_Species_vs_Fe <- data.frame(phia_cell.vol_Species_vs_Fe[,6:9], phia_cell.vol_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_cell.vol_Fe_vs_Species <- testInteractions(lm_cell.vol, fixed="Fe.level", across="Species")
phia_cell.vol_Fe_vs_Species <- phia_cell.vol_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Fe_vs_Species", tested = rownames(phia_cell.vol_Fe_vs_Species))

phia_cell.vol_Fe_vs_Species <- data.frame(phia_cell.vol_Fe_vs_Species[,6:9], phia_cell.vol_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_cell.vol_Species_vs_Cu <- testInteractions(lm_cell.vol, fixed="Species", across="Cu.level")
phia_cell.vol_Species_vs_Cu <- phia_cell.vol_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Species_vs_Cu", tested = rownames(phia_cell.vol_Species_vs_Cu))

phia_cell.vol_Species_vs_Cu <- data.frame(phia_cell.vol_Species_vs_Cu[,6:9], phia_cell.vol_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_cell.vol_Cu_vs_Species <- testInteractions(lm_cell.vol, fixed="Cu.level", across="Species")
phia_cell.vol_Cu_vs_Species <- phia_cell.vol_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "cell.vol",test_intrctn = "Cu_vs_Species", tested = rownames(phia_cell.vol_Cu_vs_Species))

phia_cell.vol_Cu_vs_Species <- data.frame(phia_cell.vol_Cu_vs_Species[,6:9], phia_cell.vol_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_cell.vol table

all_phia_cell.vol <- bind_rows(phia_cell.vol_Cu_vs_Species, phia_cell.vol_Species_vs_Cu, phia_cell.vol_Fe_vs_Species, phia_cell.vol_Species_vs_Fe, phia_cell.vol_Cu_vs_Fe, phia_cell.vol_Fe_vs_Cu, phia_cell.vol_Fe_Cu_vs_Species, phia_cell.vol_Species_Fe_vs_Cu, phia_cell.vol_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_FeDFB.cell <- anova(lm_FeDFB.cell)
anova_FeDFB.cell <- anova_FeDFB.cell %>% 
  mutate(Analysis = "anova", Data = "FeDFB.cell", Effects_Tested = rownames(anova_FeDFB.cell))

anova_FeDFB.cell <- data.frame(anova_FeDFB.cell[, 6:8], anova_FeDFB.cell[,1:5])
#~~~~~~~~~~~~~~
phia_FeDFB.cell_Species_Cu_vs_Fe <- testInteractions(lm_FeDFB.cell, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_FeDFB.cell_Species_Cu_vs_Fe <- phia_FeDFB.cell_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_FeDFB.cell_Species_Cu_vs_Fe))

phia_FeDFB.cell_Species_Cu_vs_Fe <- data.frame(phia_FeDFB.cell_Species_Cu_vs_Fe[,6:9], phia_FeDFB.cell_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_FeDFB.cell_Species_Fe_vs_Cu <- testInteractions(lm_FeDFB.cell, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_FeDFB.cell_Species_Fe_vs_Cu <- phia_FeDFB.cell_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_FeDFB.cell_Species_Fe_vs_Cu))

phia_FeDFB.cell_Species_Fe_vs_Cu <- data.frame(phia_FeDFB.cell_Species_Fe_vs_Cu[,6:9], phia_FeDFB.cell_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_FeDFB.cell_Fe_Cu_vs_Species <- testInteractions(lm_FeDFB.cell, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_FeDFB.cell_Fe_Cu_vs_Species <- phia_FeDFB.cell_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_FeDFB.cell_Fe_Cu_vs_Species))

phia_FeDFB.cell_Fe_Cu_vs_Species <- data.frame(phia_FeDFB.cell_Fe_Cu_vs_Species[,6:9], phia_FeDFB.cell_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_FeDFB.cell_Fe_vs_Cu <- testInteractions(lm_FeDFB.cell, fixed="Fe.level", across="Cu.level")
phia_FeDFB.cell_Fe_vs_Cu <- phia_FeDFB.cell_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_FeDFB.cell_Fe_vs_Cu))

phia_FeDFB.cell_Fe_vs_Cu <- data.frame(phia_FeDFB.cell_Fe_vs_Cu[,6:9], phia_FeDFB.cell_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_FeDFB.cell_Cu_vs_Fe <- testInteractions(lm_FeDFB.cell, fixed="Cu.level", across="Fe.level")
phia_FeDFB.cell_Cu_vs_Fe <- phia_FeDFB.cell_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_FeDFB.cell_Cu_vs_Fe))

phia_FeDFB.cell_Cu_vs_Fe <- data.frame(phia_FeDFB.cell_Cu_vs_Fe[,6:9], phia_FeDFB.cell_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_FeDFB.cell_Species_vs_Fe <- testInteractions(lm_FeDFB.cell, fixed="Species", across="Fe.level")
phia_FeDFB.cell_Species_vs_Fe <- phia_FeDFB.cell_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Species_vs_Fe", tested = rownames(phia_FeDFB.cell_Species_vs_Fe))

phia_FeDFB.cell_Species_vs_Fe <- data.frame(phia_FeDFB.cell_Species_vs_Fe[,6:9], phia_FeDFB.cell_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_FeDFB.cell_Fe_vs_Species <- testInteractions(lm_FeDFB.cell, fixed="Fe.level", across="Species")
phia_FeDFB.cell_Fe_vs_Species <- phia_FeDFB.cell_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Fe_vs_Species", tested = rownames(phia_FeDFB.cell_Fe_vs_Species))

phia_FeDFB.cell_Fe_vs_Species <- data.frame(phia_FeDFB.cell_Fe_vs_Species[,6:9], phia_FeDFB.cell_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_FeDFB.cell_Species_vs_Cu <- testInteractions(lm_FeDFB.cell, fixed="Species", across="Cu.level")
phia_FeDFB.cell_Species_vs_Cu <- phia_FeDFB.cell_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Species_vs_Cu", tested = rownames(phia_FeDFB.cell_Species_vs_Cu))

phia_FeDFB.cell_Species_vs_Cu <- data.frame(phia_FeDFB.cell_Species_vs_Cu[,6:9], phia_FeDFB.cell_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_FeDFB.cell_Cu_vs_Species <- testInteractions(lm_FeDFB.cell, fixed="Cu.level", across="Species")
phia_FeDFB.cell_Cu_vs_Species <- phia_FeDFB.cell_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cell",test_intrctn = "Cu_vs_Species", tested = rownames(phia_FeDFB.cell_Cu_vs_Species))

phia_FeDFB.cell_Cu_vs_Species <- data.frame(phia_FeDFB.cell_Cu_vs_Species[,6:9], phia_FeDFB.cell_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_FeDFB.cell table

all_phia_FeDFB.cell <- bind_rows(phia_FeDFB.cell_Cu_vs_Species, phia_FeDFB.cell_Species_vs_Cu, phia_FeDFB.cell_Fe_vs_Species, phia_FeDFB.cell_Species_vs_Fe, phia_FeDFB.cell_Cu_vs_Fe, phia_FeDFB.cell_Fe_vs_Cu, phia_FeDFB.cell_Fe_Cu_vs_Species, phia_FeDFB.cell_Species_Fe_vs_Cu, phia_FeDFB.cell_Species_Cu_vs_Fe)
```

####Now for FeDFB.cellvol
...
#~~~~~~~~~~~~~~
anova_FeDFB.cellvol <- anova(lm_FeDFB.cellvol)
anova_FeDFB.cellvol <- anova_FeDFB.cellvol %>% 
  mutate(Analysis = "anova", Data = "FeDFB.cellvol", Effects_Tested = rownames(anova_FeDFB.cellvol))

anova_FeDFB.cellvol <- data.frame(anova_FeDFB.cellvol[, 6:8], anova_FeDFB.cellvol[,1:5])
#~~~~~~~~~~~~~~
phia_FeDFB.cellvol_Species_Cu_vs_Fe <- testInteractions(lm_FeDFB.cellvol, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_FeDFB.cellvol_Species_Cu_vs_Fe <- phia_FeDFB.cellvol_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_FeDFB.cellvol_Species_Cu_vs_Fe))

phia_FeDFB.cellvol_Species_Cu_vs_Fe <- data.frame(phia_FeDFB.cellvol_Species_Cu_vs_Fe[,6:9], phia_FeDFB.cellvol_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_FeDFB.cellvol_Species_Fe_vs_Cu <- testInteractions(lm_FeDFB.cellvol, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_FeDFB.cellvol_Species_Fe_vs_Cu <- phia_FeDFB.cellvol_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_FeDFB.cellvol_Species_Fe_vs_Cu))

phia_FeDFB.cellvol_Species_Fe_vs_Cu <- data.frame(phia_FeDFB.cellvol_Species_Fe_vs_Cu[,6:9], phia_FeDFB.cellvol_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_FeDFB.cellvol_Fe_Cu_vs_Species <- testInteractions(lm_FeDFB.cellvol, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_FeDFB.cellvol_Fe_Cu_vs_Species <- phia_FeDFB.cellvol_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_FeDFB.cellvol_Fe_Cu_vs_Species))

phia_FeDFB.cellvol_Fe_Cu_vs_Species <- data.frame(phia_FeDFB.cellvol_Fe_Cu_vs_Species[,6:9], phia_FeDFB.cellvol_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_FeDFB.cellvol_Fe_vs_Cu <- testInteractions(lm_FeDFB.cellvol, fixed="Fe.level", across="Cu.level")
phia_FeDFB.cellvol_Fe_vs_Cu <- phia_FeDFB.cellvol_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_FeDFB.cellvol_Fe_vs_Cu))

phia_FeDFB.cellvol_Fe_vs_Cu <- data.frame(phia_FeDFB.cellvol_Fe_vs_Cu[,6:9], phia_FeDFB.cellvol_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_FeDFB.cellvol_Cu_vs_Fe <- testInteractions(lm_FeDFB.cellvol, fixed="Cu.level", across="Fe.level")
phia_FeDFB.cellvol_Cu_vs_Fe <- phia_FeDFB.cellvol_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_FeDFB.cellvol_Cu_vs_Fe))

phia_FeDFB.cellvol_Cu_vs_Fe <- data.frame(phia_FeDFB.cellvol_Cu_vs_Fe[,6:9], phia_FeDFB.cellvol_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_FeDFB.cellvol_Species_vs_Fe <- testInteractions(lm_FeDFB.cellvol, fixed="Species", across="Fe.level")
phia_FeDFB.cellvol_Species_vs_Fe <- phia_FeDFB.cellvol_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Species_vs_Fe", tested = rownames(phia_FeDFB.cellvol_Species_vs_Fe))

phia_FeDFB.cellvol_Species_vs_Fe <- data.frame(phia_FeDFB.cellvol_Species_vs_Fe[,6:9], phia_FeDFB.cellvol_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_FeDFB.cellvol_Fe_vs_Species <- testInteractions(lm_FeDFB.cellvol, fixed="Fe.level", across="Species")
phia_FeDFB.cellvol_Fe_vs_Species <- phia_FeDFB.cellvol_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Fe_vs_Species", tested = rownames(phia_FeDFB.cellvol_Fe_vs_Species))

phia_FeDFB.cellvol_Fe_vs_Species <- data.frame(phia_FeDFB.cellvol_Fe_vs_Species[,6:9], phia_FeDFB.cellvol_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_FeDFB.cellvol_Species_vs_Cu <- testInteractions(lm_FeDFB.cellvol, fixed="Species", across="Cu.level")
phia_FeDFB.cellvol_Species_vs_Cu <- phia_FeDFB.cellvol_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Species_vs_Cu", tested = rownames(phia_FeDFB.cellvol_Species_vs_Cu))

phia_FeDFB.cellvol_Species_vs_Cu <- data.frame(phia_FeDFB.cellvol_Species_vs_Cu[,6:9], phia_FeDFB.cellvol_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_FeDFB.cellvol_Cu_vs_Species <- testInteractions(lm_FeDFB.cellvol, fixed="Cu.level", across="Species")
phia_FeDFB.cellvol_Cu_vs_Species <- phia_FeDFB.cellvol_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "FeDFB.cellvol",test_intrctn = "Cu_vs_Species", tested = rownames(phia_FeDFB.cellvol_Cu_vs_Species))

phia_FeDFB.cellvol_Cu_vs_Species <- data.frame(phia_FeDFB.cellvol_Cu_vs_Species[,6:9], phia_FeDFB.cellvol_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_FeDFB.cellvol table

all_phia_FeDFB.cellvol <- bind_rows(phia_FeDFB.cellvol_Cu_vs_Species, phia_FeDFB.cellvol_Species_vs_Cu, phia_FeDFB.cellvol_Fe_vs_Species, phia_FeDFB.cellvol_Species_vs_Fe, phia_FeDFB.cellvol_Cu_vs_Fe, phia_FeDFB.cellvol_Fe_vs_Cu, phia_FeDFB.cellvol_Fe_Cu_vs_Species, phia_FeDFB.cellvol_Species_Fe_vs_Cu, phia_FeDFB.cellvol_Species_Cu_vs_Fe)
```


####Now for 14C.alpha.cell
...
#~~~~~~~~~~~~~~
anova_14C.alpha.cell <- anova(lm_14C.alpha.cell)
anova_14C.alpha.cell <- anova_14C.alpha.cell %>% 
  mutate(Analysis = "anova", Data = "14C.alpha.cell", Effects_Tested = rownames(anova_14C.alpha.cell))

anova_14C.alpha.cell <- data.frame(anova_14C.alpha.cell[, 6:8], anova_14C.alpha.cell[,1:5])
#~~~~~~~~~~~~~~
phia_14C.alpha.cell_Species_Cu_vs_Fe <- testInteractions(lm_14C.alpha.cell, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_14C.alpha.cell_Species_Cu_vs_Fe <- phia_14C.alpha.cell_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_14C.alpha.cell_Species_Cu_vs_Fe))

phia_14C.alpha.cell_Species_Cu_vs_Fe <- data.frame(phia_14C.alpha.cell_Species_Cu_vs_Fe[,6:9], phia_14C.alpha.cell_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_14C.alpha.cell_Species_Fe_vs_Cu <- testInteractions(lm_14C.alpha.cell, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_14C.alpha.cell_Species_Fe_vs_Cu <- phia_14C.alpha.cell_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_14C.alpha.cell_Species_Fe_vs_Cu))

phia_14C.alpha.cell_Species_Fe_vs_Cu <- data.frame(phia_14C.alpha.cell_Species_Fe_vs_Cu[,6:9], phia_14C.alpha.cell_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_14C.alpha.cell_Fe_Cu_vs_Species <- testInteractions(lm_14C.alpha.cell, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_14C.alpha.cell_Fe_Cu_vs_Species <- phia_14C.alpha.cell_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_14C.alpha.cell_Fe_Cu_vs_Species))

phia_14C.alpha.cell_Fe_Cu_vs_Species <- data.frame(phia_14C.alpha.cell_Fe_Cu_vs_Species[,6:9], phia_14C.alpha.cell_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_14C.alpha.cell_Fe_vs_Cu <- testInteractions(lm_14C.alpha.cell, fixed="Fe.level", across="Cu.level")
phia_14C.alpha.cell_Fe_vs_Cu <- phia_14C.alpha.cell_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_14C.alpha.cell_Fe_vs_Cu))

phia_14C.alpha.cell_Fe_vs_Cu <- data.frame(phia_14C.alpha.cell_Fe_vs_Cu[,6:9], phia_14C.alpha.cell_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_14C.alpha.cell_Cu_vs_Fe <- testInteractions(lm_14C.alpha.cell, fixed="Cu.level", across="Fe.level")
phia_14C.alpha.cell_Cu_vs_Fe <- phia_14C.alpha.cell_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_14C.alpha.cell_Cu_vs_Fe))

phia_14C.alpha.cell_Cu_vs_Fe <- data.frame(phia_14C.alpha.cell_Cu_vs_Fe[,6:9], phia_14C.alpha.cell_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_14C.alpha.cell_Species_vs_Fe <- testInteractions(lm_14C.alpha.cell, fixed="Species", across="Fe.level")
phia_14C.alpha.cell_Species_vs_Fe <- phia_14C.alpha.cell_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Species_vs_Fe", tested = rownames(phia_14C.alpha.cell_Species_vs_Fe))

phia_14C.alpha.cell_Species_vs_Fe <- data.frame(phia_14C.alpha.cell_Species_vs_Fe[,6:9], phia_14C.alpha.cell_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_14C.alpha.cell_Fe_vs_Species <- testInteractions(lm_14C.alpha.cell, fixed="Fe.level", across="Species")
phia_14C.alpha.cell_Fe_vs_Species <- phia_14C.alpha.cell_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Fe_vs_Species", tested = rownames(phia_14C.alpha.cell_Fe_vs_Species))

phia_14C.alpha.cell_Fe_vs_Species <- data.frame(phia_14C.alpha.cell_Fe_vs_Species[,6:9], phia_14C.alpha.cell_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_14C.alpha.cell_Species_vs_Cu <- testInteractions(lm_14C.alpha.cell, fixed="Species", across="Cu.level")
phia_14C.alpha.cell_Species_vs_Cu <- phia_14C.alpha.cell_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Species_vs_Cu", tested = rownames(phia_14C.alpha.cell_Species_vs_Cu))

phia_14C.alpha.cell_Species_vs_Cu <- data.frame(phia_14C.alpha.cell_Species_vs_Cu[,6:9], phia_14C.alpha.cell_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_14C.alpha.cell_Cu_vs_Species <- testInteractions(lm_14C.alpha.cell, fixed="Cu.level", across="Species")
phia_14C.alpha.cell_Cu_vs_Species <- phia_14C.alpha.cell_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.cell",test_intrctn = "Cu_vs_Species", tested = rownames(phia_14C.alpha.cell_Cu_vs_Species))

phia_14C.alpha.cell_Cu_vs_Species <- data.frame(phia_14C.alpha.cell_Cu_vs_Species[,6:9], phia_14C.alpha.cell_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_14C.alpha.cell table

all_phia_14C.alpha.cell <- bind_rows(phia_14C.alpha.cell_Cu_vs_Species, phia_14C.alpha.cell_Species_vs_Cu, phia_14C.alpha.cell_Fe_vs_Species, phia_14C.alpha.cell_Species_vs_Fe, phia_14C.alpha.cell_Cu_vs_Fe, phia_14C.alpha.cell_Fe_vs_Cu, phia_14C.alpha.cell_Fe_Cu_vs_Species, phia_14C.alpha.cell_Species_Fe_vs_Cu, phia_14C.alpha.cell_Species_Cu_vs_Fe)
```

####Now for 14C.alpha.chla
...
#~~~~~~~~~~~~~~
anova_14C.alpha.chla <- anova(lm_14C.alpha.chla)
anova_14C.alpha.chla <- anova_14C.alpha.chla %>% 
  mutate(Analysis = "anova", Data = "14C.alpha.chla", Effects_Tested = rownames(anova_14C.alpha.chla))

anova_14C.alpha.chla <- data.frame(anova_14C.alpha.chla[, 6:8], anova_14C.alpha.chla[,1:5])
#~~~~~~~~~~~~~~
phia_14C.alpha.chla_Species_Cu_vs_Fe <- testInteractions(lm_14C.alpha.chla, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_14C.alpha.chla_Species_Cu_vs_Fe <- phia_14C.alpha.chla_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_14C.alpha.chla_Species_Cu_vs_Fe))

phia_14C.alpha.chla_Species_Cu_vs_Fe <- data.frame(phia_14C.alpha.chla_Species_Cu_vs_Fe[,6:9], phia_14C.alpha.chla_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_14C.alpha.chla_Species_Fe_vs_Cu <- testInteractions(lm_14C.alpha.chla, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_14C.alpha.chla_Species_Fe_vs_Cu <- phia_14C.alpha.chla_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_14C.alpha.chla_Species_Fe_vs_Cu))

phia_14C.alpha.chla_Species_Fe_vs_Cu <- data.frame(phia_14C.alpha.chla_Species_Fe_vs_Cu[,6:9], phia_14C.alpha.chla_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_14C.alpha.chla_Fe_Cu_vs_Species <- testInteractions(lm_14C.alpha.chla, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_14C.alpha.chla_Fe_Cu_vs_Species <- phia_14C.alpha.chla_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_14C.alpha.chla_Fe_Cu_vs_Species))

phia_14C.alpha.chla_Fe_Cu_vs_Species <- data.frame(phia_14C.alpha.chla_Fe_Cu_vs_Species[,6:9], phia_14C.alpha.chla_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_14C.alpha.chla_Fe_vs_Cu <- testInteractions(lm_14C.alpha.chla, fixed="Fe.level", across="Cu.level")
phia_14C.alpha.chla_Fe_vs_Cu <- phia_14C.alpha.chla_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_14C.alpha.chla_Fe_vs_Cu))

phia_14C.alpha.chla_Fe_vs_Cu <- data.frame(phia_14C.alpha.chla_Fe_vs_Cu[,6:9], phia_14C.alpha.chla_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_14C.alpha.chla_Cu_vs_Fe <- testInteractions(lm_14C.alpha.chla, fixed="Cu.level", across="Fe.level")
phia_14C.alpha.chla_Cu_vs_Fe <- phia_14C.alpha.chla_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_14C.alpha.chla_Cu_vs_Fe))

phia_14C.alpha.chla_Cu_vs_Fe <- data.frame(phia_14C.alpha.chla_Cu_vs_Fe[,6:9], phia_14C.alpha.chla_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_14C.alpha.chla_Species_vs_Fe <- testInteractions(lm_14C.alpha.chla, fixed="Species", across="Fe.level")
phia_14C.alpha.chla_Species_vs_Fe <- phia_14C.alpha.chla_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Species_vs_Fe", tested = rownames(phia_14C.alpha.chla_Species_vs_Fe))

phia_14C.alpha.chla_Species_vs_Fe <- data.frame(phia_14C.alpha.chla_Species_vs_Fe[,6:9], phia_14C.alpha.chla_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_14C.alpha.chla_Fe_vs_Species <- testInteractions(lm_14C.alpha.chla, fixed="Fe.level", across="Species")
phia_14C.alpha.chla_Fe_vs_Species <- phia_14C.alpha.chla_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Fe_vs_Species", tested = rownames(phia_14C.alpha.chla_Fe_vs_Species))

phia_14C.alpha.chla_Fe_vs_Species <- data.frame(phia_14C.alpha.chla_Fe_vs_Species[,6:9], phia_14C.alpha.chla_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_14C.alpha.chla_Species_vs_Cu <- testInteractions(lm_14C.alpha.chla, fixed="Species", across="Cu.level")
phia_14C.alpha.chla_Species_vs_Cu <- phia_14C.alpha.chla_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Species_vs_Cu", tested = rownames(phia_14C.alpha.chla_Species_vs_Cu))

phia_14C.alpha.chla_Species_vs_Cu <- data.frame(phia_14C.alpha.chla_Species_vs_Cu[,6:9], phia_14C.alpha.chla_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_14C.alpha.chla_Cu_vs_Species <- testInteractions(lm_14C.alpha.chla, fixed="Cu.level", across="Species")
phia_14C.alpha.chla_Cu_vs_Species <- phia_14C.alpha.chla_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.alpha.chla",test_intrctn = "Cu_vs_Species", tested = rownames(phia_14C.alpha.chla_Cu_vs_Species))

phia_14C.alpha.chla_Cu_vs_Species <- data.frame(phia_14C.alpha.chla_Cu_vs_Species[,6:9], phia_14C.alpha.chla_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_14C.alpha.chla table

all_phia_14C.alpha.chla <- bind_rows(phia_14C.alpha.chla_Cu_vs_Species, phia_14C.alpha.chla_Species_vs_Cu, phia_14C.alpha.chla_Fe_vs_Species, phia_14C.alpha.chla_Species_vs_Fe, phia_14C.alpha.chla_Cu_vs_Fe, phia_14C.alpha.chla_Fe_vs_Cu, phia_14C.alpha.chla_Fe_Cu_vs_Species, phia_14C.alpha.chla_Species_Fe_vs_Cu, phia_14C.alpha.chla_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_14C.ek.chla <- anova(lm_14C.ek.chla)
anova_14C.ek.chla <- anova_14C.ek.chla %>% 
  mutate(Analysis = "anova", Data = "14C.ek.chla", Effects_Tested = rownames(anova_14C.ek.chla))

anova_14C.ek.chla <- data.frame(anova_14C.ek.chla[, 6:8], anova_14C.ek.chla[,1:5])
#~~~~~~~~~~~~~~
phia_14C.ek.chla_Species_Cu_vs_Fe <- testInteractions(lm_14C.ek.chla, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_14C.ek.chla_Species_Cu_vs_Fe <- phia_14C.ek.chla_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_14C.ek.chla_Species_Cu_vs_Fe))

phia_14C.ek.chla_Species_Cu_vs_Fe <- data.frame(phia_14C.ek.chla_Species_Cu_vs_Fe[,6:9], phia_14C.ek.chla_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_14C.ek.chla_Species_Fe_vs_Cu <- testInteractions(lm_14C.ek.chla, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_14C.ek.chla_Species_Fe_vs_Cu <- phia_14C.ek.chla_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_14C.ek.chla_Species_Fe_vs_Cu))

phia_14C.ek.chla_Species_Fe_vs_Cu <- data.frame(phia_14C.ek.chla_Species_Fe_vs_Cu[,6:9], phia_14C.ek.chla_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_14C.ek.chla_Fe_Cu_vs_Species <- testInteractions(lm_14C.ek.chla, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_14C.ek.chla_Fe_Cu_vs_Species <- phia_14C.ek.chla_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_14C.ek.chla_Fe_Cu_vs_Species))

phia_14C.ek.chla_Fe_Cu_vs_Species <- data.frame(phia_14C.ek.chla_Fe_Cu_vs_Species[,6:9], phia_14C.ek.chla_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_14C.ek.chla_Fe_vs_Cu <- testInteractions(lm_14C.ek.chla, fixed="Fe.level", across="Cu.level")
phia_14C.ek.chla_Fe_vs_Cu <- phia_14C.ek.chla_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_14C.ek.chla_Fe_vs_Cu))

phia_14C.ek.chla_Fe_vs_Cu <- data.frame(phia_14C.ek.chla_Fe_vs_Cu[,6:9], phia_14C.ek.chla_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_14C.ek.chla_Cu_vs_Fe <- testInteractions(lm_14C.ek.chla, fixed="Cu.level", across="Fe.level")
phia_14C.ek.chla_Cu_vs_Fe <- phia_14C.ek.chla_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_14C.ek.chla_Cu_vs_Fe))

phia_14C.ek.chla_Cu_vs_Fe <- data.frame(phia_14C.ek.chla_Cu_vs_Fe[,6:9], phia_14C.ek.chla_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_14C.ek.chla_Species_vs_Fe <- testInteractions(lm_14C.ek.chla, fixed="Species", across="Fe.level")
phia_14C.ek.chla_Species_vs_Fe <- phia_14C.ek.chla_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Species_vs_Fe", tested = rownames(phia_14C.ek.chla_Species_vs_Fe))

phia_14C.ek.chla_Species_vs_Fe <- data.frame(phia_14C.ek.chla_Species_vs_Fe[,6:9], phia_14C.ek.chla_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_14C.ek.chla_Fe_vs_Species <- testInteractions(lm_14C.ek.chla, fixed="Fe.level", across="Species")
phia_14C.ek.chla_Fe_vs_Species <- phia_14C.ek.chla_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Fe_vs_Species", tested = rownames(phia_14C.ek.chla_Fe_vs_Species))

phia_14C.ek.chla_Fe_vs_Species <- data.frame(phia_14C.ek.chla_Fe_vs_Species[,6:9], phia_14C.ek.chla_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_14C.ek.chla_Species_vs_Cu <- testInteractions(lm_14C.ek.chla, fixed="Species", across="Cu.level")
phia_14C.ek.chla_Species_vs_Cu <- phia_14C.ek.chla_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Species_vs_Cu", tested = rownames(phia_14C.ek.chla_Species_vs_Cu))

phia_14C.ek.chla_Species_vs_Cu <- data.frame(phia_14C.ek.chla_Species_vs_Cu[,6:9], phia_14C.ek.chla_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_14C.ek.chla_Cu_vs_Species <- testInteractions(lm_14C.ek.chla, fixed="Cu.level", across="Species")
phia_14C.ek.chla_Cu_vs_Species <- phia_14C.ek.chla_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "14C.ek.chla",test_intrctn = "Cu_vs_Species", tested = rownames(phia_14C.ek.chla_Cu_vs_Species))

phia_14C.ek.chla_Cu_vs_Species <- data.frame(phia_14C.ek.chla_Cu_vs_Species[,6:9], phia_14C.ek.chla_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_14C.ek.chla table

all_phia_14C.ek.chla <- bind_rows(phia_14C.ek.chla_Cu_vs_Species, phia_14C.ek.chla_Species_vs_Cu, phia_14C.ek.chla_Fe_vs_Species, phia_14C.ek.chla_Species_vs_Fe, phia_14C.ek.chla_Cu_vs_Fe, phia_14C.ek.chla_Fe_vs_Cu, phia_14C.ek.chla_Fe_Cu_vs_Species, phia_14C.ek.chla_Species_Fe_vs_Cu, phia_14C.ek.chla_Species_Cu_vs_Fe)
```
...
```r
lm_AOX <- lm(data = mydata, AOXactivity ~ (Species * Fe.level * Cu.level))


#~~~~~~~~~~~~~~
anova_AOX <- anova(lm_AOX)
anova_AOX <- anova_AOX %>% 
  mutate(Analysis = "anova", Data = "AOX", Effects_Tested = rownames(anova_AOX))

anova_AOX <- data.frame(anova_AOX[, 6:8], anova_AOX[,1:5])
#~~~~~~~~~~~~~~
phia_AOX_Species_Cu_vs_Fe <- testInteractions(lm_AOX, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_AOX_Species_Cu_vs_Fe <- phia_AOX_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_AOX_Species_Cu_vs_Fe))

phia_AOX_Species_Cu_vs_Fe <- data.frame(phia_AOX_Species_Cu_vs_Fe[,6:9], phia_AOX_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_AOX_Species_Fe_vs_Cu <- testInteractions(lm_AOX, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_AOX_Species_Fe_vs_Cu <- phia_AOX_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_AOX_Species_Fe_vs_Cu))

phia_AOX_Species_Fe_vs_Cu <- data.frame(phia_AOX_Species_Fe_vs_Cu[,6:9], phia_AOX_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_AOX_Fe_Cu_vs_Species <- testInteractions(lm_AOX, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_AOX_Fe_Cu_vs_Species <- phia_AOX_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_AOX_Fe_Cu_vs_Species))

phia_AOX_Fe_Cu_vs_Species <- data.frame(phia_AOX_Fe_Cu_vs_Species[,6:9], phia_AOX_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_AOX_Fe_vs_Cu <- testInteractions(lm_AOX, fixed="Fe.level", across="Cu.level")
phia_AOX_Fe_vs_Cu <- phia_AOX_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_AOX_Fe_vs_Cu))

phia_AOX_Fe_vs_Cu <- data.frame(phia_AOX_Fe_vs_Cu[,6:9], phia_AOX_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_AOX_Cu_vs_Fe <- testInteractions(lm_AOX, fixed="Cu.level", across="Fe.level")
phia_AOX_Cu_vs_Fe <- phia_AOX_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_AOX_Cu_vs_Fe))

phia_AOX_Cu_vs_Fe <- data.frame(phia_AOX_Cu_vs_Fe[,6:9], phia_AOX_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_AOX_Species_vs_Fe <- testInteractions(lm_AOX, fixed="Species", across="Fe.level")
phia_AOX_Species_vs_Fe <- phia_AOX_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Species_vs_Fe", tested = rownames(phia_AOX_Species_vs_Fe))

phia_AOX_Species_vs_Fe <- data.frame(phia_AOX_Species_vs_Fe[,6:9], phia_AOX_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_AOX_Fe_vs_Species <- testInteractions(lm_AOX, fixed="Fe.level", across="Species")
phia_AOX_Fe_vs_Species <- phia_AOX_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Fe_vs_Species", tested = rownames(phia_AOX_Fe_vs_Species))

phia_AOX_Fe_vs_Species <- data.frame(phia_AOX_Fe_vs_Species[,6:9], phia_AOX_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_AOX_Species_vs_Cu <- testInteractions(lm_AOX, fixed="Species", across="Cu.level")
phia_AOX_Species_vs_Cu <- phia_AOX_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Species_vs_Cu", tested = rownames(phia_AOX_Species_vs_Cu))

phia_AOX_Species_vs_Cu <- data.frame(phia_AOX_Species_vs_Cu[,6:9], phia_AOX_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_AOX_Cu_vs_Species <- testInteractions(lm_AOX, fixed="Cu.level", across="Species")
phia_AOX_Cu_vs_Species <- phia_AOX_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "AOX",test_intrctn = "Cu_vs_Species", tested = rownames(phia_AOX_Cu_vs_Species))

phia_AOX_Cu_vs_Species <- data.frame(phia_AOX_Cu_vs_Species[,6:9], phia_AOX_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_AOX table

all_phia_AOX <- bind_rows(phia_AOX_Cu_vs_Species, phia_AOX_Species_vs_Cu, phia_AOX_Fe_vs_Species, phia_AOX_Species_vs_Fe, phia_AOX_Cu_vs_Fe, phia_AOX_Fe_vs_Cu, phia_AOX_Fe_Cu_vs_Species, phia_AOX_Species_Fe_vs_Cu, phia_AOX_Species_Cu_vs_Fe)
```
...
```r
lm_O2.Chla <- lm(data = mydata, GrossPchla.mol.O2.mol.Chla..h. ~ (Species * Fe.level * Cu.level))


#~~~~~~~~~~~~~~
anova_O2.Chla <- anova(lm_O2.Chla)
anova_O2.Chla <- anova_O2.Chla %>% 
  mutate(Analysis = "anova", Data = "O2.Chla", Effects_Tested = rownames(anova_O2.Chla))

anova_O2.Chla <- data.frame(anova_O2.Chla[, 6:8], anova_O2.Chla[,1:5])
#~~~~~~~~~~~~~~
phia_O2.Chla_Species_Cu_vs_Fe <- testInteractions(lm_O2.Chla, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_O2.Chla_Species_Cu_vs_Fe <- phia_O2.Chla_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_O2.Chla_Species_Cu_vs_Fe))

phia_O2.Chla_Species_Cu_vs_Fe <- data.frame(phia_O2.Chla_Species_Cu_vs_Fe[,6:9], phia_O2.Chla_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_O2.Chla_Species_Fe_vs_Cu <- testInteractions(lm_O2.Chla, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_O2.Chla_Species_Fe_vs_Cu <- phia_O2.Chla_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_O2.Chla_Species_Fe_vs_Cu))

phia_O2.Chla_Species_Fe_vs_Cu <- data.frame(phia_O2.Chla_Species_Fe_vs_Cu[,6:9], phia_O2.Chla_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_O2.Chla_Fe_Cu_vs_Species <- testInteractions(lm_O2.Chla, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_O2.Chla_Fe_Cu_vs_Species <- phia_O2.Chla_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_O2.Chla_Fe_Cu_vs_Species))

phia_O2.Chla_Fe_Cu_vs_Species <- data.frame(phia_O2.Chla_Fe_Cu_vs_Species[,6:9], phia_O2.Chla_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_O2.Chla_Fe_vs_Cu <- testInteractions(lm_O2.Chla, fixed="Fe.level", across="Cu.level")
phia_O2.Chla_Fe_vs_Cu <- phia_O2.Chla_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_O2.Chla_Fe_vs_Cu))

phia_O2.Chla_Fe_vs_Cu <- data.frame(phia_O2.Chla_Fe_vs_Cu[,6:9], phia_O2.Chla_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_O2.Chla_Cu_vs_Fe <- testInteractions(lm_O2.Chla, fixed="Cu.level", across="Fe.level")
phia_O2.Chla_Cu_vs_Fe <- phia_O2.Chla_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_O2.Chla_Cu_vs_Fe))

phia_O2.Chla_Cu_vs_Fe <- data.frame(phia_O2.Chla_Cu_vs_Fe[,6:9], phia_O2.Chla_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_O2.Chla_Species_vs_Fe <- testInteractions(lm_O2.Chla, fixed="Species", across="Fe.level")
phia_O2.Chla_Species_vs_Fe <- phia_O2.Chla_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Species_vs_Fe", tested = rownames(phia_O2.Chla_Species_vs_Fe))

phia_O2.Chla_Species_vs_Fe <- data.frame(phia_O2.Chla_Species_vs_Fe[,6:9], phia_O2.Chla_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_O2.Chla_Fe_vs_Species <- testInteractions(lm_O2.Chla, fixed="Fe.level", across="Species")
phia_O2.Chla_Fe_vs_Species <- phia_O2.Chla_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Fe_vs_Species", tested = rownames(phia_O2.Chla_Fe_vs_Species))

phia_O2.Chla_Fe_vs_Species <- data.frame(phia_O2.Chla_Fe_vs_Species[,6:9], phia_O2.Chla_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_O2.Chla_Species_vs_Cu <- testInteractions(lm_O2.Chla, fixed="Species", across="Cu.level")
phia_O2.Chla_Species_vs_Cu <- phia_O2.Chla_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Species_vs_Cu", tested = rownames(phia_O2.Chla_Species_vs_Cu))

phia_O2.Chla_Species_vs_Cu <- data.frame(phia_O2.Chla_Species_vs_Cu[,6:9], phia_O2.Chla_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_O2.Chla_Cu_vs_Species <- testInteractions(lm_O2.Chla, fixed="Cu.level", across="Species")
phia_O2.Chla_Cu_vs_Species <- phia_O2.Chla_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Chla",test_intrctn = "Cu_vs_Species", tested = rownames(phia_O2.Chla_Cu_vs_Species))

phia_O2.Chla_Cu_vs_Species <- data.frame(phia_O2.Chla_Cu_vs_Species[,6:9], phia_O2.Chla_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_O2.Chla table

all_phia_O2.Chla <- bind_rows(phia_O2.Chla_Cu_vs_Species, phia_O2.Chla_Species_vs_Cu, phia_O2.Chla_Fe_vs_Species, phia_O2.Chla_Species_vs_Fe, phia_O2.Chla_Cu_vs_Fe, phia_O2.Chla_Fe_vs_Cu, phia_O2.Chla_Fe_Cu_vs_Species, phia_O2.Chla_Species_Fe_vs_Cu, phia_O2.Chla_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_O2.Cell <- anova(lm_O2.Cell)
anova_O2.Cell <- anova_O2.Cell %>% 
  mutate(Analysis = "anova", Data = "O2.Cell", Effects_Tested = rownames(anova_O2.Cell))

anova_O2.Cell <- data.frame(anova_O2.Cell[, 6:8], anova_O2.Cell[,1:5])
#~~~~~~~~~~~~~~
phia_O2.Cell_Species_Cu_vs_Fe <- testInteractions(lm_O2.Cell, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_O2.Cell_Species_Cu_vs_Fe <- phia_O2.Cell_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_O2.Cell_Species_Cu_vs_Fe))

phia_O2.Cell_Species_Cu_vs_Fe <- data.frame(phia_O2.Cell_Species_Cu_vs_Fe[,6:9], phia_O2.Cell_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_O2.Cell_Species_Fe_vs_Cu <- testInteractions(lm_O2.Cell, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_O2.Cell_Species_Fe_vs_Cu <- phia_O2.Cell_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_O2.Cell_Species_Fe_vs_Cu))

phia_O2.Cell_Species_Fe_vs_Cu <- data.frame(phia_O2.Cell_Species_Fe_vs_Cu[,6:9], phia_O2.Cell_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_O2.Cell_Fe_Cu_vs_Species <- testInteractions(lm_O2.Cell, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_O2.Cell_Fe_Cu_vs_Species <- phia_O2.Cell_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_O2.Cell_Fe_Cu_vs_Species))

phia_O2.Cell_Fe_Cu_vs_Species <- data.frame(phia_O2.Cell_Fe_Cu_vs_Species[,6:9], phia_O2.Cell_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_O2.Cell_Fe_vs_Cu <- testInteractions(lm_O2.Cell, fixed="Fe.level", across="Cu.level")
phia_O2.Cell_Fe_vs_Cu <- phia_O2.Cell_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_O2.Cell_Fe_vs_Cu))

phia_O2.Cell_Fe_vs_Cu <- data.frame(phia_O2.Cell_Fe_vs_Cu[,6:9], phia_O2.Cell_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_O2.Cell_Cu_vs_Fe <- testInteractions(lm_O2.Cell, fixed="Cu.level", across="Fe.level")
phia_O2.Cell_Cu_vs_Fe <- phia_O2.Cell_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_O2.Cell_Cu_vs_Fe))

phia_O2.Cell_Cu_vs_Fe <- data.frame(phia_O2.Cell_Cu_vs_Fe[,6:9], phia_O2.Cell_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_O2.Cell_Species_vs_Fe <- testInteractions(lm_O2.Cell, fixed="Species", across="Fe.level")
phia_O2.Cell_Species_vs_Fe <- phia_O2.Cell_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Species_vs_Fe", tested = rownames(phia_O2.Cell_Species_vs_Fe))

phia_O2.Cell_Species_vs_Fe <- data.frame(phia_O2.Cell_Species_vs_Fe[,6:9], phia_O2.Cell_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_O2.Cell_Fe_vs_Species <- testInteractions(lm_O2.Cell, fixed="Fe.level", across="Species")
phia_O2.Cell_Fe_vs_Species <- phia_O2.Cell_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Fe_vs_Species", tested = rownames(phia_O2.Cell_Fe_vs_Species))

phia_O2.Cell_Fe_vs_Species <- data.frame(phia_O2.Cell_Fe_vs_Species[,6:9], phia_O2.Cell_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_O2.Cell_Species_vs_Cu <- testInteractions(lm_O2.Cell, fixed="Species", across="Cu.level")
phia_O2.Cell_Species_vs_Cu <- phia_O2.Cell_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Species_vs_Cu", tested = rownames(phia_O2.Cell_Species_vs_Cu))

phia_O2.Cell_Species_vs_Cu <- data.frame(phia_O2.Cell_Species_vs_Cu[,6:9], phia_O2.Cell_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_O2.Cell_Cu_vs_Species <- testInteractions(lm_O2.Cell, fixed="Cu.level", across="Species")
phia_O2.Cell_Cu_vs_Species <- phia_O2.Cell_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell",test_intrctn = "Cu_vs_Species", tested = rownames(phia_O2.Cell_Cu_vs_Species))

phia_O2.Cell_Cu_vs_Species <- data.frame(phia_O2.Cell_Cu_vs_Species[,6:9], phia_O2.Cell_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_O2.Cell table

all_phia_O2.Cell <- bind_rows(phia_O2.Cell_Cu_vs_Species, phia_O2.Cell_Species_vs_Cu, phia_O2.Cell_Fe_vs_Species, phia_O2.Cell_Species_vs_Fe, phia_O2.Cell_Cu_vs_Fe, phia_O2.Cell_Fe_vs_Cu, phia_O2.Cell_Fe_Cu_vs_Species, phia_O2.Cell_Species_Fe_vs_Cu, phia_O2.Cell_Species_Cu_vs_Fe)
```
...
#~~~~~~~~~~~~~~
anova_O2.Cell.vol <- anova(lm_O2.Cell.vol)
anova_O2.Cell.vol <- anova_O2.Cell.vol %>% 
  mutate(Analysis = "anova", Data = "O2.Cell.vol", Effects_Tested = rownames(anova_O2.Cell.vol))

anova_O2.Cell.vol <- data.frame(anova_O2.Cell.vol[, 6:8], anova_O2.Cell.vol[,1:5])
#~~~~~~~~~~~~~~
phia_O2.Cell.vol_Species_Cu_vs_Fe <- testInteractions(lm_O2.Cell.vol, fixed=c("Species", "Cu.level"), across="Fe.level")
phia_O2.Cell.vol_Species_Cu_vs_Fe <- phia_O2.Cell.vol_Species_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Species_Cu_vs_Fe", tested = rownames(phia_O2.Cell.vol_Species_Cu_vs_Fe))

phia_O2.Cell.vol_Species_Cu_vs_Fe <- data.frame(phia_O2.Cell.vol_Species_Cu_vs_Fe[,6:9], phia_O2.Cell.vol_Species_Cu_vs_Fe[, 1:5])
#~~~~~~~~~~~~~~

phia_O2.Cell.vol_Species_Fe_vs_Cu <- testInteractions(lm_O2.Cell.vol, fixed=c("Species", "Fe.level"), across="Cu.level")
phia_O2.Cell.vol_Species_Fe_vs_Cu <- phia_O2.Cell.vol_Species_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Species_Fe_vs_Cu", tested = rownames(phia_O2.Cell.vol_Species_Fe_vs_Cu))

phia_O2.Cell.vol_Species_Fe_vs_Cu <- data.frame(phia_O2.Cell.vol_Species_Fe_vs_Cu[,6:9], phia_O2.Cell.vol_Species_Fe_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~

phia_O2.Cell.vol_Fe_Cu_vs_Species <- testInteractions(lm_O2.Cell.vol, fixed=c("Fe.level", "Cu.level"), across="Species")
phia_O2.Cell.vol_Fe_Cu_vs_Species <- phia_O2.Cell.vol_Fe_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Fe_Cu_vs_Species", tested = rownames(phia_O2.Cell.vol_Fe_Cu_vs_Species))

phia_O2.Cell.vol_Fe_Cu_vs_Species <- data.frame(phia_O2.Cell.vol_Fe_Cu_vs_Species[,6:9], phia_O2.Cell.vol_Fe_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Cu
phia_O2.Cell.vol_Fe_vs_Cu <- testInteractions(lm_O2.Cell.vol, fixed="Fe.level", across="Cu.level")
phia_O2.Cell.vol_Fe_vs_Cu <- phia_O2.Cell.vol_Fe_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Fe_vs_Cu", tested = rownames(phia_O2.Cell.vol_Fe_vs_Cu))

phia_O2.Cell.vol_Fe_vs_Cu <- data.frame(phia_O2.Cell.vol_Fe_vs_Cu[,6:9], phia_O2.Cell.vol_Fe_vs_Cu[, 1:5])
#~~~~~~~~~~~~~~
#Cu vs Fe
phia_O2.Cell.vol_Cu_vs_Fe <- testInteractions(lm_O2.Cell.vol, fixed="Cu.level", across="Fe.level")
phia_O2.Cell.vol_Cu_vs_Fe <- phia_O2.Cell.vol_Cu_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Cu_vs_Fe", tested = rownames(phia_O2.Cell.vol_Cu_vs_Fe))

phia_O2.Cell.vol_Cu_vs_Fe <- data.frame(phia_O2.Cell.vol_Cu_vs_Fe[,6:9], phia_O2.Cell.vol_Cu_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Fe
phia_O2.Cell.vol_Species_vs_Fe <- testInteractions(lm_O2.Cell.vol, fixed="Species", across="Fe.level")
phia_O2.Cell.vol_Species_vs_Fe <- phia_O2.Cell.vol_Species_vs_Fe %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Species_vs_Fe", tested = rownames(phia_O2.Cell.vol_Species_vs_Fe))

phia_O2.Cell.vol_Species_vs_Fe <- data.frame(phia_O2.Cell.vol_Species_vs_Fe[,6:9], phia_O2.Cell.vol_Species_vs_Fe[, 1:5])

#~~~~~~~~~~~~~~
#Fe vs Species
phia_O2.Cell.vol_Fe_vs_Species <- testInteractions(lm_O2.Cell.vol, fixed="Fe.level", across="Species")
phia_O2.Cell.vol_Fe_vs_Species <- phia_O2.Cell.vol_Fe_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Fe_vs_Species", tested = rownames(phia_O2.Cell.vol_Fe_vs_Species))

phia_O2.Cell.vol_Fe_vs_Species <- data.frame(phia_O2.Cell.vol_Fe_vs_Species[,6:9], phia_O2.Cell.vol_Fe_vs_Species[, 1:5])

#~~~~~~~~~~~~~~
#Species vs Cu
phia_O2.Cell.vol_Species_vs_Cu <- testInteractions(lm_O2.Cell.vol, fixed="Species", across="Cu.level")
phia_O2.Cell.vol_Species_vs_Cu <- phia_O2.Cell.vol_Species_vs_Cu %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Species_vs_Cu", tested = rownames(phia_O2.Cell.vol_Species_vs_Cu))

phia_O2.Cell.vol_Species_vs_Cu <- data.frame(phia_O2.Cell.vol_Species_vs_Cu[,6:9], phia_O2.Cell.vol_Species_vs_Cu[, 1:5])

#~~~~~~~~~~~~~~
#Cu vs Species
phia_O2.Cell.vol_Cu_vs_Species <- testInteractions(lm_O2.Cell.vol, fixed="Cu.level", across="Species")
phia_O2.Cell.vol_Cu_vs_Species <- phia_O2.Cell.vol_Cu_vs_Species %>% 
  mutate(Analysis = "phia_interaction", Data = "O2.Cell.vol",test_intrctn = "Cu_vs_Species", tested = rownames(phia_O2.Cell.vol_Cu_vs_Species))

phia_O2.Cell.vol_Cu_vs_Species <- data.frame(phia_O2.Cell.vol_Cu_vs_Species[,6:9], phia_O2.Cell.vol_Cu_vs_Species[, 1:5])

#~~~~~~~~~~~~~~~~~~~

#making the big Phia_O2.Cell.vol table
