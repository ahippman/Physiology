...
```{r lm_all_Growthrate.percent_TO1003, eval=TRUE}
...
```{r phia_interaction means_Growthrate.percent_TO1003, eval=TRUE}
...
```{r phia_interaction_Contrasts_Fe.level_Cu.level_Growthrate.percent_TO1003, eval=TRUE}
testInteractions(lm_TO03_Growthrate.percent, fixed="Fe.level", across="Cu.level")
```

This means that there is significant growthrate changes from high Cu to low Cu in both Species (F(1,18) = 99.67, p.val < 0.00001) and that the same is true under low Fe conditions, that the additional lowering of Cu.level changes growth rates significantly (F(1,18) = 37.90, p.val < 0.00001)

```{r phia_interaction_Contrasts_Cu.level_Fe.level_Growthrate.percent_TO1003, eval=TRUE}
...
```{r lm_all_cell.SA.um2, eval=TRUE}
...
```{r phia_interaction means_cell.SA.um2, eval=TRUE}
...
First: Species against Fe + Cu level
```{r phia_interaction_Contrasts_Species_Felevel_cell.SA.um2, eval=TRUE}
testInteractions(lm_cell.SA.um2, fixed="Species", across="Fe.level")
```


```{r phia_interaction_Contrasts_Felevel_and_Species_cell.SA.um2, eval=TRUE}
testInteractions(lm_cell.SA.um2, fixed = "Fe.level", across="Species")
```

Interaction testing for copper
```{r phia_interaction_Contrasts_2fixed_against1_cell.SA.um2, eval=TRUE}
...
```{r graph_cell.SA.Vol.ratio, eval=TRUE}
...
```{r lm_all_cell.SA.Vol.ratio, eval=TRUE}
...
```{r lm_cell.SA.Vol.ratio, eval=TRUE}
...
```{r phia_interaction means_cell.SA.Vol.ratio, eval=TRUE}
...
First: Species against Fe + Cu level
```{r phia_interaction_Contrasts_Species_Felevel_cell.SA.Vol.ratio, eval=TRUE}
testInteractions(lm_cell.SA.Vol.ratio, fixed="Species", across="Fe.level")
```


```{r phia_interaction_Contrasts_Felevel_and_Species_cell.SA.Vol.ratio, eval=TRUE}
testInteractions(lm_cell.SA.Vol.ratio, fixed = "Fe.level", across="Species")
```

Interaction testing for copper
```{r phia_interaction_Contrasts_2fixed_against1_cell.SA.Vol.ratio, eval=TRUE}
...
```{r phia_interaction_Contrasts_FeAndCulevel_vs_Species_FeDFB.cell, eval=TRUE}
...
```{r graph_14C.pmax.chla, eval=TRUE}
...
```{r lm_all_14C.pmax.chla, eval=TRUE}
...
```{r phia_interaction means_14C.pmax.chla, eval=TRUE}
...
*First: Testing the three differnet variables against each other:*
```{r phia_interaction_Contrasts_Species_Culevel_and_Felevel_14C.pmax.chla, eval=TRUE}
...
*Second: Species : Cu level*
```{r phia_interaction_Contrasts_Species_Culevel_14C.pmax.chla, eval=TRUE}
testInteractions(lm_14C.pmax.chla, fixed="Species", across="Cu.level") # no significance
```



```{r phia_interaction_Contrasts_Culevel_and_Species_14C.pmax.chla, eval=TRUE}
...
```{r phia_interaction_Contrasts_lowCu_related_14C.pmax.chla, eval=TRUE}
...
```{r graph_14C.155uEinstein.chla, eval=TRUE}
p <- ggplot (mydata , aes(Treatment, X14C.per.Chla.at.155uE))
p + geom_point(aes(group=Merged, colour=Species), size = 3, alpha=0.5)+
...
```{r lm_all_14C.155uEinstein.chla, eval=TRUE}
...
```{r lm_14C.155uEinstein.chla, eval=TRUE}
...
```{r phia_interaction means_14C.155uEinstein.chla, eval=TRUE}
...
*First: Testing the three differnet variables against each other:*
```{r phia_interaction_Contrasts_Species_Culevel_and_Felevel_14C.155uEinstein.chla, eval=TRUE}
...
*Second: Species : Cu level*
```{r phia_interaction_Contrasts_Species_Culevel_14C.155uEinstein.chla, eval=TRUE}
...
```{r phia_interaction_Contrasts_Culevel_and_Species_14C.155uEinstein.chla, eval=TRUE}
...
let's check the 2 fixed and against one other, especially in regards to low Cu
```{r phia_interaction_Contrasts_lowCu_related_14C.155uEinstein.chla, eval=TRUE}
...
```{r stat_SummaryTable_cell.size, eval = FALSE}
lm_cell.size <- lm (data = mydata, cell.size..um ~ (Species + Fe.level + Cu.level)^2 - Species:Cu.level - Fe.level:Cu.level)
...
```{r stat_SummaryTable_cell.vol, eval = FALSE}
lm_cell.vol <- lm (data = mydata, cell.volume.fl.cell ~ (Species + Fe.level + Cu.level)^2 - Species:Cu.level - Fe.level:Cu.level)
...
```{r stat_SummaryTable_FeDFB.cell, eval = FALSE}
...
```{r stat_SummaryTable_FeDFB.cellvol, eval = FALSE}
...
```{r stat_SummaryTable_14C.alpha.cell, eval = FALSE}
...
```{r stat_SummaryTable_14C.alpha.chla, eval = FALSE}
...
```{r stat_SummaryTable_14C.ek.chla, eval = FALSE}
...
```{r stat_SummaryTable_AOX, eval = FALSE}
...
```{r stat_SummaryTable_O2.Chla, eval = FALSE}
...
```{r stat_SummaryTable_O2.Cell, eval = FALSE}
...
```{r stat_SummaryTable_O2.Cell.vol, eval = FALSE}
...
```{r adding significanca conlumn, eval=FALSE}
